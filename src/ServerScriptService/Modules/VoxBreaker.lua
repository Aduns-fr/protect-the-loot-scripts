...
if PartCacheEnabled then
	...
	PartFolder.Name = "VoxelPartCache"
	PartFolder:SetAttribute("VoxelPartCache", true)
...
local function stripExecutableContent(root: Instance): boolean
...
local function clonePartWithoutScripts(part: Part): Part
...
local function CutPartinHalf(block : Part, TimeToReset : number) --Cuts part into two evenly shaped pieces.
	...
	for _, offsetVector in pairs(bipolarVectorSet) do
		...
		if PartCacheEnabled then
			...
			clone = clonePartWithoutScripts(block)
		end
	...
	task.spawn(function()
		if rs:IsClient() then
			...
			connection2 = self.Part.AttributeChanged:Connect(function(attribute)
				if self.Part:GetAttribute("Active") == false
					or self.Part:GetAttribute("Unweld") == true then
					connection:Disconnect()
					connection2:Disconnect()
				end
			end)	
...
function VoxBreaker:ReturnPart(Part:Part) --Returns Part to PartCache
	...
	elseif not cache or not table.find(cache.Open, Part) then
		Part:Destroy()
	end
end

function VoxBreaker:GetCacheStats()
	if not cache then
		return 0, 0
	end
	return #cache.Open, #cache.InUse
end

function VoxBreaker:DiscardPart(Part: Part)
	if cache then
		cache:DiscardPart(Part)
	end
end
